...
p = subprocess.Popen('sacct --jobs=\"'+",".join(slurmids)+'\" --format=\"JobID, Start, Elapsed, MaxRSS, NNodes, NTasks\"', shell=True, stdout=subprocess.PIPE)
...
for di, dr in enumerate(diruse):
    ...
    for k in dr.split("_"):
        if k.endswith("src"): Nsrcs[ind] = int(k[:-3])
        if k.endswith("time"): Ntimes[ind] = int(k[:-4])
        if k.endswith('chan'): Nchans[ind] = int(k[:-4])
        if k == 'mwa': Nbls[ind] = 8128
        if k == 'triangle' : Nbls[ind] = 3
