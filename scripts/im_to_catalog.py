...
parser.add_argument(
    "-s", dest="spacing", type=float, default=1.0, help="source spacing in degrees"
)
parser.add_argument(
    "-n",
...
# fix to be a useful date and not 2045
...
parser.add_argument(
    "--array_location", type=str, default="-30,116", help="LAT,LON in degrees"
)
...
dpi = int(np.ceil(args.char_pix_height / emsize))
...
im_cmd = [
    ...
    "white",
    "-font",
    "C059-Bold",  # "Keyboard",  # still looking for optimal font
    "-pointsize",
    "10",
    "-units",
    "PixelsPerInch",
...
with open(catfile, "w") as outfile:
    ...
    outstr += "#SOURCE_ID\tRA_ICRS [deg]\tDec_ICRS [deg]\tFlux [Jy]\tFrequency [Hz]\n"
    for i in range(len(icrs_coord)):
        outstr += f"HERATEST{i}" + "\t"
        # switched ra.hour to ra.degree
        # outstr += str(np.round(icrs_coord[i].ra.hour, 5)) + "\t"
        outstr += str(np.round(icrs_coord[i].ra.degree, 6)) + "\t"
