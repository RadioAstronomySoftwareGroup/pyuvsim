...
parser = argparse.ArgumentParser(description=("make a test pattern catalog that spells out something"))
...
dpi = np.int(np.ceil(args.char_pix_height / emsize))  #height of a letter in pixels/hight of letter in inches
text = args.text.upper()
im_cmd = ["convert",
        "-background", "black" ,
        "-fill", "white",
         "-font", "Keyboard",#keyboard, Helvetica, skia also are "ok" but not great. still looking for optimal font
         "-pointsize", "10",
         "-units", "PixelsPerInch",
         "-density", str(dpi),
         "-trim", "+repage",
