...
parser.add_argument(
    "yaml_file", type=str, help="Settings file.", default="settings.yaml"
)
parser.add_argument(
    "--init_configs",
    action="store_true",
...
parser.add_argument(
    "--update_log", action="store_true", help="Update the log of benchmarking runs."
)
parser.add_argument(
    "-o",
    "--outdir",
...
if args.submit and args.update_log:
    raise ValueError(
        "Cannot submit benchmarking job and update the log at the same time."
...
if args.submit:
    output = check_call(
        [
            "sbatch",
            "-o",
            os.path.join(args.outdir, "slurm_%A.out"),
            settings["jobscript"],
        ]
    )
