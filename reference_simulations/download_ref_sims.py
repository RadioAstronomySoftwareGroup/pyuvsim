...
try:
    ...
    raise ImportError(
        "pooche required for full script functionality"
    ) from e
...
def main():
    ...
    parser = argparse.ArgumentParser(description="a list of reference simulations to download")

    # TODO: flip flop on if default is caching or not, and similarly on if a script available for simulation comparison should grab cache or local
    # TODO: the comparison script should honestly search both
    parser.add_argument("--cache", action="store_true", help="Caches files to pooch pyuvsim cache if passed, otherwise downloads to local directory.")
    ...
    parser.add_argument(
        ...
        help="List of reference simulations to download via keyword. Downloads all available files if"
    ...
    for sim in sims:
        if sim in ref_sims:
            ...
            print(f"sim \"{sim}\" not found! Check the available keys in the ref_sims list"
